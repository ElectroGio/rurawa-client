...
const STEP_STATUS_ICON = {
    [COMPLETE]: <HiCheck className='text-secondary'/>,
    [PENDING]: null,
    [IN_PROGRESS]: null,
    [ERROR]: <HiX />,
}
...
const StepItem = forwardRef<HTMLDivElement, StepItemProps>((props, ref) => {
    ...
    const stepIconClass = classNames(
        ...
        status === COMPLETE && `bg-white text-white`,
        status === ERROR && `step-item-icon-error`,
        status === IN_PROGRESS &&
            `text-white dark:text-gray-100 border-white step-item-icon-current bg-[#734D90]`,
    )
    ...
    const stepConnectClass = classNames(
        ...
        status === COMPLETE ? `bg-white` : `inactive`,
    )
    ...
    const stepTitleClass = classNames(
        ...
        status === IN_PROGRESS && `text-white dark:text-gray-100`,
        onStepChange && status !== ERROR && `hover:text-white`,
    )
    ...
    return (
        ...
        >
            <div className={stepWrapperClass}>
                ...
                <div className={stepIconClass}>{status === COMPLETE && stepIcon}</div>
                {title && (
